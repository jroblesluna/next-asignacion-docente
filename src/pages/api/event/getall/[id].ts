  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const result = await pool.request().input('id', id).query(`
       ...
       FORMAT(fechaCambio, 'HH:mm') AS "time",descripcion as description, tipoEvento as name, estado FROM  ad_evento 
	   WHERE periodo=202506 order by estado asc, 'date' desc , 'time' desc`);
